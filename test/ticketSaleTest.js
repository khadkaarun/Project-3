...
contract("TicketSale", (accounts) => {
  ...
  it("should allow a user to buy a ticket", async () => {
    ...
    assert.equal(ticketOwner.toNumber(), 1, "Alice should own ticket #1");
  });
  ...
  it("should return the ticket ID owned by an address", async () => {
    ...
    assert.equal(ticketId.toNumber(), 2, "Bob should own ticket #2");
  });
  ...
  it("should allow users to swap tickets", async function() {
    ...
    assert.equal(aliceTicket.toNumber(), 4, "Alice should now own ticket #4");
    assert.equal(bobTicket.toNumber(), 3, "Bob should now own ticket #3");
  });
  ...
  it("should allow a user to resale their ticket and another user to buy it", async function() {
    ...
    assert.equal(ticketOwner.toNumber(), 5, "Claire should now own ticket #5");

    const aliceTicket = await instance.getTicketOf(alice);
    assert.equal(aliceTicket.toNumber(), 0, "Alice should no longer own a ticket");
  });
  ...
  it("should display resale tickets and their prices", async () => {
    ...
    assert.equal(resaleTickets[0].toNumber(), 6, "Ticket #6 should be on resale");
  });
});
